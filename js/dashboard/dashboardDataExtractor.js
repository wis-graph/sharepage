...
import { getNotePath, getRawUrl } from '../services/pathService.js?v=41000';
import { transformObsidianImageLinks, parseFrontmatter } from '../services/markdownService.js?v=41000';
...
export async function extractNoteFromLink(link) {
  ...
  const path = getNotePath(link.replace(/\.md$/, ''));
  ...
  try {
    ...
    const enrichedNote = {
      ...
      thumbnail: thumbnail,
      tags: metadata.tags || [],
      path: getNotePath(note.file.replace(/\.md$/, '')) // Use centralized path function
    };
